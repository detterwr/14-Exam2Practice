...
class Box(object):
    ...
    def append_string(self, additional_contents):
        ...
        num2append = min(space, len(additional_contents))
        to_add = ''
        for k in range(num2append):
            to_add = to_add + additional_contents[k]
        self.contents = self.contents + to_add

        to_return = ''
        for k in range(num2append, len(additional_contents)):
            to_return = to_return + additional_contents[k]
    ...
    def shrink(self, new_volume):
        ...
        if new_volume >= len(self.contents):
            ...
            for l in range(new_volume, len(self.contents)):
                remainder = remainder + self.contents[l]
            self.volume = new_volume
            self.contents = new
            return remainder
